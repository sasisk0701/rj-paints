...
export function downloadBillPdf(options: BillPdfOptions) {
  ...
  const rows = options.items.length > 0 ? options.items.slice(0, 6) : [{ description: 'Transaction', amount: options.totalAmount }];
  ...
  // Leave blank ruled rows to retain a familiar printed shop-bill appearance.
  while (rowTop > 404) {
    const rowBottom = rowTop - 30;
    rect(tableLeft, rowBottom, tableWidth, 30, undefined, true);
    for (let i = 1; i < colX.length - 1; i += 1) line(colX[i], rowBottom, colX[i], rowTop);
    rowTop = rowBottom;
  }
